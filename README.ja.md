...
英語版・日本語版の両チャーターは `v0.4` に整合済みで、第9章に `ai-lmr-guard v0.4` 由来の実行ガード拡張プロファイルを反映しています。
...
- `LMR_AI_Charter_private_public_v0.1.md`
  - English version (v0.4)
- `LMR_AI_Charter_private_public_v0.1_ja.md`
  - 日本語版（v0.4、第9章に実行ガード拡張プロファイルを統合）
...
- 削除・消去・初期化をAI自動化で最も危険な操作群として扱う原則
- 確認メッセージのテンプレート
- ログの安全ルール（最小化・マスキング）
- 実行レーン向け拡張: レーン分類、STEP 0.5意図解析、hard block 条件、shadow mode 記録
...
- `Policy Layer`（運用ポリシー）
  - `LMR_AI_Charter_private_public_v0.1_ja.md`（または `LMR_AI_Charter_private_public_v0.1.md`）を基準文書として採用し、組織ルールに合わせて改訂する
...
1. まず `LMR_AI_Charter_private_public_v0.1_ja.md`（または英語版 `LMR_AI_Charter_private_public_v0.1.md`）を基準文書として採用する
2. 自組織の運用に合わせて重み・閾値・例外条件を調整する
3. システムプロンプトには文書内の「短縮版」を使う
4. 実装には文書内の「実装用の最小サンプル（擬似ルール）」を起点にする
5. 実行系自動化では、日本語版第9章（v0.4追補）を優先して適用する

## LLMからLMRを参照する方法

LLMにガード内容を参照させる最も簡単な方法は、GitHub上の `SKILL.md` をURLで指定することです。
...
jsDelivr を使った CDN 形式:

```text
https://cdn.jsdelivr.net/gh/buzzlyhan/lmr-ai-charter@main/ai-lmr-guard/SKILL.md
```

開発・検証では `main` 参照で問題ありません。本番運用のエージェントでは、安全基準が意図せず変わらないように commit hash または release tag で固定することを推奨します。
...
```text
外部送信、公開、削除、契約/金銭処理、認証情報、権限変更を実行する前に、
次の LMR guard を読み込み、その内容に従うこと:
https://raw.githubusercontent.com/buzzlyhan/lmr-ai-charter/main/ai-lmr-guard/SKILL.md

ファイルを読み込めない場合、高リスク操作は CONFIRM または BLOCK を既定にすること。
```

LLM実行環境がURL取得やブラウズに対応していない場合は、起動時に `SKILL.md` を取得して system/developer prompt に注入してください。トークン制約が強い環境では、憲章内の短縮版を埋め込み、全文はURLを正本として参照する運用が現実的です。
...
- 削除系操作は、対象・範囲・復旧可能性・明示承認が揃わない限り、全文削除やHD/ディスクフォーマットへ拡張しないでください
...
- Japanese: `LMR_AI_Charter_private_public_v0.1_ja.md`
- English: `LMR_AI_Charter_private_public_v0.1.md`
